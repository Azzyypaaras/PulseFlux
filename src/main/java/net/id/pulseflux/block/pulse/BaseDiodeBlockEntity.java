...
import net.id.pulseflux.block.base.PulseBlockEntity;
...
public class BaseDiodeBlockEntity extends PulseBlockEntity implements IoProvider {

    @NotNull private Optional<RelativeObjectData<PulseIo>> child = Optional.empty();
    private int renderTicks = -25;


    public BaseDiodeBlockEntity(Material material, BlockPos pos, BlockState state) {
    ...
    public static FabricBlockEntityTypeBuilder.Factory<BaseDiodeBlockEntity> factory(Material material) {
        return ((blockPos, blockState) -> new BaseDiodeBlockEntity(material, blockPos, blockState));
    }
